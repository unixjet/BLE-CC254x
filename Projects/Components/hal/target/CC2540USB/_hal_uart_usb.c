...
#define BUF_SIZE 512
__no_init static uint8 halUartRxQ[BUF_SIZE];
__no_init static uint8 halUartTxQ[BUF_SIZE];
...
static uint8 HalUARTRx(uint8 *buf, uint8 max);
//#endif
...
//#if ! defined ( HAL_UART_CUSTOM ) 
static uint8 HalUARTTx(uint8 *buf, uint8 cnt);
//#endif
